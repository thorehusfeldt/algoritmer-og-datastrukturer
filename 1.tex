...
\ForFromTo{i}{0}{n-1} \\
  \>{\rm læg $a_i$, $b_i$, $c$ sammen, resultatet er $s_i$ og den nye mente $c$}\\
$s_n\Is c$
\end{code}
\end{quote}
...
De tre søjler i tabellen til \emph{venstre} angiver $n$, udførelsestiden $T_n$ af \protect\CC-implementationen fra afsnit~\lref{s: programs}  og forholdet $T_n/T_{n/2}$.  
...
Vi vil nu beskrive hans metode i detaljer.
Lad $k=\floor{n/2}$. 
...
Tallene $a_\mathrm H$, $a_\mathrm L$, $b_\mathrm H$, $b_\mathrm L$, $a_\mathrm H + a_\mathrm L$ og $b_\mathrm H + b_\mathrm L$ består hvert af højst $\ceil{n/2} + 1$ cifre. 
...
I lemma~\lref{thm:recursive school help} har vi set, at det maksimale antal elementaroperationer, som skal bruges til multiplikation af to $n$-cifrede tal, opfylder rekursionsligningen
\begin{equation*}
T(n) \le 
\begin{cases}
1\,,   & \text{når $n = 1$}\,; \\
...
Er Karatsubas algoritme den hurtigst kendte multiplikationsalgoritme?
